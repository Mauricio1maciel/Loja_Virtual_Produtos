...
export default function LojaVirtual() {
  ...
  const sincronizarFornecedor = async () => {
    setLoadingSync(true);
    setMensagemSync("");
    try {
      const response = await Api.Api.post(
        "/api/integracao/fornecedor/sincronizar"
      );
      setMensagemSync("Sincronização realizada com sucesso!");
      console.log("Resposta da sincronização:", response.data);
      carregarProdutos(); // Atualiza os produtos após sincronizar
    } catch (error) {
      console.error("Erro na sincronização:", error);
      setMensagemSync("Erro ao sincronizar. Veja o console para detalhes.");
    } finally {
            ...
            {(localStorage.getItem("isAdmin") === "true" &&
              ...
              <>
                <button
                  ...
                  onClick={sincronizarFornecedor}
                  disabled={loadingSync}
                >
                  {loadingSync ? "Sincronizando..." : "Sincronizar Fornecedor"}
                </button>
                {mensagemSync && (
                  <span className="text-success fw-semibold ms-2">{mensagemSync}</span>
                )}
              </>
            )}
          </div>
              ...
              <div className="card h-100 shadow-sm">
                <div className="position-relative">
                  <img
                    src={
                      produto.imagemUrl ||
                      "https://placehold.co/250x250?text=Sem+Imagem"
                    }
                    className="card-img-top object-fit-contain"
                    alt={produto.nomeProduto}
                    style={{ height: "250px", objectFit: "contain", padding: "1rem" }}
